...
for(loop in 1:loopnum)
{
  ...
  SCISS_Aug_Re <- SCISS_Aug(S_y_supervise,S_x_supervise,S_x_unsupervise)
  SCISS_PoS_Re <- SCISS_PoS(S_y_supervise,S_x_supervise,S_x_unsupervise,"poisson")
  theta_dr <- density_ratio(S_x_supervise,S_x_unsupervise,S_y_supervise)
  
  path <- paste(save_path,loop,'.rda',sep='')
  
  save(Graph=Graph2,SCISS_Aug_Re,SCISS_PoS_Re,theta_dr,file=path)
}
